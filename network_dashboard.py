...
# DEBUG: Confirm this updated code is running
print("DEBUG: Updated network_dashboard.py is running!")
...
def fetch_graylog_data():
    ...
    try:
        # Use the working endpoint we confirmed exists
        url = f"{GRAYLOG_URL}/api/system/inputs"
        ...
        if response.status_code == 200:
            ...
            if inputs:
                st.success(f"✅ Connected to Graylog - Found {len(inputs)} system inputs")
                # Convert inputs to a format the dashboard can display
                messages = []
                for input_item in inputs:
                    messages.append({
                        'message': f"Input: {input_item.get('title', 'Unknown')} - {input_item.get('name', 'Unknown')}",
                        'source': input_item.get('override_source', 'Graylog'),
                        'timestamp': input_item.get('created_at', 'Unknown')
                    })
                return messages
            else:
                st.warning("⚠️ Connected to Graylog but no inputs found")
                return []
        else:
