...
def fetch_graylog_data():
    ...
    try:
        # Now that we know connection works, use the search API
        ...
        if response.status_code == 200:
            ...
            if messages:
                st.success(f"✅ Connected to Graylog - Found {len(messages)} messages")
                return messages
            else:
                st.warning("⚠️ Connected to Graylog but no messages found for the selected period")
                return []
        else:
