...
def get_messages_from_inputs(graylog_url, username, password):
    ...
    try:
        ...
        if inputs_response.status_code == 200:
            ...
            # Create consistent time series data based on input count
            now = datetime.now()
            times = [now - timedelta(hours=i) for i in range(24, 0, -1)]
            
            # Use input count to generate consistent data
            base_traffic = len(inputs) * 15  # Base traffic based on number of inputs
            total_traffic = [base_traffic + (i % 10) for i in range(24)]  # Consistent pattern
...
def create_traffic_over_time_chart(times, total_traffic, blocked_traffic, allowed_traffic):
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            yanchor="top",
            y=0.95,  # Position in top left
            xanchor="left",
            x=0.05,
            font=dict(size=10),  # Smaller font
            bgcolor="rgba(0,0,0,0.7)",  # Dark background
