...
def get_graylog_data(time_period):
    ...
    try:
        ...
        # Query Graylog for messages
        query_url = f"{graylog_url}/api/search/universal/relative"
        headers = {
            'Authorization': f'Basic {requests.utils.b64encode(f"{username}:{password}".encode()).decode()}',
            'Content-Type': 'application/json'
        }
        
        params = {
            'query': '*',  # Search all messages
            'range': '3600',  # Last hour (will be overridden by from/to)
